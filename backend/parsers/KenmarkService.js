...
/**
 * KENMARK SERVICE - UPC-Based API Enrichment
 *
 * This service provides a complete solution for processing Kenmark orders:
 * 1. Parse HTML → Extract order info and frame data with UPCs from image URLs
 * 2. Enrich → Query Kenmark API using UPC codes for pricing, stock, and product details
 * 3. Validate → Cross-reference with Kenmark API data
 *
 ...
 *                   The last segment (715317146401) is the UPC/barcode
 *
 * Backend: Uses jiecosystem.net (same as L'amyamerica and Modern Optical)
 *
 * Usage:
 *   const service = new KenmarkService();
 *   const result = await service.enrichOrderData(parsedData);
 */
class KenmarkService {
    constructor(options = {}) {
        this.config = {
            // API Configuration
            apiUrl: 'https://kenmarkeyewear.com/US/api/CatalogAPI/filter',
    ...
    log(...args) {
        if (this.config.debug) {
            console.log('[KenmarkService]', ...args);
        }
    }
     ...
     * @param {Object} parsedData - Data from kenmarkParser.js
    ...
    async enrichOrderData(parsedData) {
        ...
        try {
            console.log('🔍 Enriching Kenmark order data...\n');
            ...
            console.error('❌ KenmarkService error:', error.message);
            throw error;
        }
    }
    ...
    async processItem(item, index, total) {
        ...
        const enrichedData = validation.bestMatch ? {
            ...
            collectionName: validation.bestMatch.collectionName
        } : null;
    ...
    clearCache() {
        this.cache.clear();
        console.log('🗑️  Kenmark cache cleared');
    }
}

module.exports = KenmarkService;
