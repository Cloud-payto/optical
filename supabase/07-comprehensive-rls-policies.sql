...
-- ============================================================
-- HELPER FUNCTION: Get Account ID for Current User
-- ============================================================
...
COMMENT ON FUNCTION auth.get_user_account_id() IS 'Returns the account_id for the currently authenticated user. Used in RLS policies.';
...
USING (
  return_report_id IN (
    ...
    WHERE account_id = auth.get_user_account_id()
  )
);
...
BEGIN
  ...
  RAISE NOTICE '  ✓ Efficient auth checks using helper function';
  RAISE NOTICE '';
END $$;
